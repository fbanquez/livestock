...
class livestock_weighing(models.Model):
    ...
    _order = "animal_id, created_date desc"
    ...
    def _daily_gain_compute(self):
        ...
        if self.animal_id.id:
            ...
            if len(reg) > 1:
                ...
                self.daily_gain = (float(reg[1][1]) - float(reg[0][1]))/diff.days
    ...
    def _life_gain_compute(self):
        ...
        if self.animal_id.id:
            ...
            if reg:
                diff = datetime.strptime(self.created_date,'%Y-%m-%d %H:%M:%S') - datetime.strptime(reg[1], '%Y-%m-%d')
                self.life_gain = (self.current_weight - float(reg[0]))/diff.days
    ...
    def _weight_compute(self):
        ...
        if self.animal_id.id:
            ...
            if reg:
                diff = datetime.strptime(self.created_date,'%Y-%m-%d %H:%M:%S') - datetime.strptime(reg[1], '%Y-%m-%d')
                self.adjusted_weight = ((self.current_weight - float(reg[0]))/diff.days)*(self.projecting_days + float(reg[0]))
    ...
    created_date = fields.Datetime(string='Date', required=True, default=datetime.now(), help="Date on which the weight event occurred")
